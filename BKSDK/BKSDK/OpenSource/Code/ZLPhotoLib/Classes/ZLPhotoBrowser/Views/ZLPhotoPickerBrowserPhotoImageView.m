...
- (void) addGesture{
    ...
    // 上下滑动
//    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanChange:)];
...
- (void)handlePanChange:(UIPanGestureRecognizer *)pan {
    if ([_tapDelegate respondsToSelector:@selector(imageView:panChangeDetected:)]) {
        [_tapDelegate imageView:self panChangeDetected:pan];
    }
}
